...
<ParamField body="metadata.content" type="array" required>
  ...
  - `frames`: Seedance 1.5 Pro does not support using `duration` and `frames` together; either works, and `frames` takes priority. If you want fractional-second videos, specify `frames`. Range: all integers within `[29, 289]` that match `25 + 4n` where `n` is a positive integer
  - `--fps` or `--framespersecond`: Frame rate, only supports `24`
