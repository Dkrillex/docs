...
<ParamField body="metadata" type="object">
  Doubao models use `metadata.content` array to pass content, supporting mixed text and image input. Parameters are controlled through special markers in text prompts:
...
<ParamField body="image" type="string">
  Reference image (supports URL or Base64 format). In `metadata.content`, images can be marked via `role` field:
