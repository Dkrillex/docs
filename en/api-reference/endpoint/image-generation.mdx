...
<ParamField body="model" type="string" required>
  ...
  - GPT Image series: `gpt-image-1`, `gpt-image-1-mini`, etc.
...
System.out.println(result.getData().get(0).getB64Json());
```
  </Tab>
...
<Tabs>
  <Tab title="Doubao Seedream">
    ...
    <ParamField body="optimize_prompt_options" type="object">
      Prompt optimization options, only supported by `doubao-seedream-4.0` and `doubao-seedream-4.5`:
      - `mode` (string): Optimization mode
        ...
        - `"creative"`: Creative mode (only 4.5)
        - `"precise"`: Precise mode (only 4.5)
    </ParamField>
  </Tab>
  ...
  <Tab title="Gemini">
    <ParamField body="size" type="string">
      Image aspect ratio, supports: `1:1`, `3:2`, `2:3`, `3:4`, `4:3`, `4:5`, `5:4`, `9:16`, `16:9`, `21:9`. Can also use pixel dimensions (e.g., `1024x1024`), system will automatically convert to corresponding ratio
    </ParamField>
  ...
  <Tab title="Tongyi Qianwen">
    <ParamField body="parameters" type="object" required>
      ...
      - `size` (string, required): Image size, supports: `1328*1328`, `1664*928`, `928*1664`, `1472*1140`, `1140*1472`
      - `negative_prompt` (string): Negative prompt, used to exclude unwanted elements
      - `prompt_extend` (boolean, default: true): Whether to enable prompt extension. Recommended for short prompts, not recommended for detailed prompts
      - `watermark` (boolean, default: true): Whether to add watermark
      - `n` (integer, only qwen-image-edit-plus): Number of images to generate, range `1-6`, default `1`
      - `seed` (integer, only qwen-image-edit-plus): Random seed, range `0-2147483647`
    </ParamField>
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="Doubao Seedream">
    <Tabs>
      ...
      <Tab title="4.5 Model (Creative/Precise Mode)">
        ```bash
        # Creative mode
        curl -X POST "https://llm.ai-nebula.com/v1/images/generations" \
          -H "Authorization: Bearer sk-xxxxxxxxxx" \
      ...
      <Tab title="Image Editing">
        ...
        curl -X POST "https://llm.ai-nebula.com/v1/images/generations" \
          ...
          -d '{
            ...
            "prompt": "Change \"Hello\" in the image to \"你好\", keep other content unchanged",
            "parameters": {
              "n": 1,
              "negative_prompt": "blurry text, typos",
              "watermark": false,
              "seed": 123456
            },
            "contents": [
              {
                ...
                "parts": [
                  {"image": "data:image/png;base64,iVBORw0KGgoAAxxxx..."},
                  {"text": "Change \"Hello\" in the image to \"你好\""}
                ]
              }
            ]
          }'
...
### Multi-Image Fusion (Nano Banana Exclusive)

Nano Banana supports simultaneous input of 2-5 images for fusion generation:
...
### Text Rendering (Tongyi Qianwen Exclusive)

Tongyi Qianwen excels at generating complex text in images, supporting Chinese and English text rendering:
...
curl -X POST "https://llm.ai-nebula.com/v1/images/generations" \
  ...
  -d '{
    "model": "qwen-image-plus",
    "prompt": "A coffee poster with large title \"Artisan Coffee\" at the top, subtitle \"Aromatic Every Moment\", with a steaming cup of coffee in the middle",
    "parameters": {
      "size": "928*1664",
...
### Guidance Scale Control (Doubao Seedream 3.0 Exclusive)

Use the `guidance_scale` parameter to control how closely the generated image matches the prompt:
...
- Higher `guidance_scale` values make the generated image strictly follow the prompt
- Recommended range: `1.0-10.0`, default: `2.5`
- Lower values (e.g., `1.0-3.0`): More creative freedom, more varied results
- Higher values (e.g., `7.0-10.0`): Stricter prompt matching, more precise results

### Random Seed Control (Doubao Seedream Series)

Use the `seed` parameter to control the randomness of generation results. Same seed produces similar results:
...
**Parameter Description**:
- `seed` value range: `0` to `2147483647`
- Using the same `seed` and `prompt` can produce similar (but not guaranteed identical) results
- If `seed` is not provided, the system will use a random seed, and each generation will produce different results

### Sequential Image Generation (Doubao Seedream 4.x Exclusive)

Doubao Seedream 4.0 and 4.5 support sequential image generation, allowing you to generate multiple stylistically consistent images at once:
...
**Parameter Description**:
- `sequential_image_generation`: Set to `"auto"` to enable sequential image generation
- `sequential_image_generation_options.max_images`: Number of images, range `1-4`, default `4`
- Sequential image generation produces multiple stylistically consistent images, suitable for scenarios requiring series images

### Prompt Optimization (Doubao Seedream 4.0/4.5 Exclusive)

Doubao Seedream 4.0 and 4.5 support prompt optimization options to automatically optimize prompt effects:

```bash
# Standard mode (supported by both 4.0 and 4.5)
curl -X POST "https://llm.ai-nebula.com/v1/images/generations" \
  -H "Authorization: Bearer sk-xxxxxxxxxx" \
  -H "Content-Type: application/json" \
  -d '{
...
**Parameter Description**:
- `mode: "standard"`: Standard mode, higher quality but longer time (default, supported by both 4.0 and 4.5)
- `mode: "fast"`: Fast mode, shorter time but average quality (only 4.0)
- `mode: "creative"`: Creative mode, more creative freedom (only 4.5)
- `mode: "precise"`: Precise mode, more precise prompt matching (only 4.5)

### Multi-Image Input (GPT Image Exclusive)

GPT Image supports simultaneous input of multiple images (up to 10) for fusion generation:
...
**Parameter Description**:
- `images`: Array of multiple input images, supports up to 10 images
- `input_fidelity`: Controls the fidelity of input images
  - `"low"`: More creative freedom, larger differences from original image
  - `"medium"`: Balance between fidelity and creativity
  - `"high"`: Preserve original image features, smaller changes
  - `"auto"`: Automatically select appropriate fidelity
- `n`: Number of images to generate, range `1-10`, each image consumes corresponding quota

### Image Editing (Tongyi Qianwen Image Editing)

Use the `qwen-image-edit-plus` model for image editing, supporting text modification, object addition/removal, style transfer, etc.:
...
**Parameter Description**:
- `parameters.n`: Number of images to generate, range `1-6`, default `1`. Billed per image when generating multiple variants
- `parameters.seed`: Random seed, range `0-2147483647`, used to control the randomness of generation results
- `parameters.negative_prompt`: Negative prompt, used to exclude unwanted elements, e.g., "blurry text, typos"
- `parameters.watermark`: Whether to add watermark, default `true`
