...
<ParamField body="model" type="string" required>
  ...
  - GPT Image 系列：`gpt-image-1`、`gpt-image-1-mini` 等
...
<Tabs>
  <Tab title="豆包 Seedream">
    ...
    <ParamField body="optimize_prompt_options" type="object">
      提示词优化选项，仅 `doubao-seedream-4.0` 和 `doubao-seedream-4.5` 支持：
      - `mode` (string): 优化模式
        - `"standard"`: 标准模式，质量更高但耗时较长（默认，4.0 和 4.5 都支持）
        - `"fast"`: 快速模式，耗时更短但质量一般（仅 4.0 支持）
        - `"creative"`: 创意模式（仅 4.5 支持）
        - `"precise"`: 精确模式（仅 4.5 支持）
    </ParamField>
  </Tab>
  ...
  <Tab title="通义千问">
    <ParamField body="parameters" type="object" required>
      ...
      - `size` (string, 必填): 图片尺寸，支持：`1328*1328`、`1664*928`、`928*1664`、`1472*1140`、`1140*1472`
      - `negative_prompt` (string): 负面提示词，用于排除不想要的元素
      - `prompt_extend` (boolean, default: true): 是否启用提示词扩展。简短提示词建议启用，详细提示词建议关闭
      - `watermark` (boolean, default: true): 是否添加水印
      - `n` (integer, 仅 qwen-image-edit-plus): 生成数量，范围 `1-6`，默认 `1`
      - `seed` (integer, 仅 qwen-image-edit-plus): 随机种子，范围 `0-2147483647`
    </ParamField>
  </Tab>
</Tabs>
...
### 引导系数控制（豆包 Seedream 3.0 特有）

使用 `guidance_scale` 参数控制生成图像与提示词的匹配程度：
...
**参数说明**：
- `guidance_scale` 值越高，生成图像越严格遵循提示词
- 建议范围：`1.0-10.0`，默认：`2.5`
- 值较低时（如 `1.0-3.0`）：更多创意发挥，结果更自由
- 值较高时（如 `7.0-10.0`）：更严格匹配提示词，结果更精确

### 随机种子控制（豆包 Seedream 系列）

使用 `seed` 参数控制生成结果的随机性，相同 seed 会产生相似的结果：
...
**参数说明**：
- `seed` 取值范围：`0` 到 `2147483647`
- 使用相同的 `seed` 和 `prompt` 可以获得相似（但不保证完全相同）的结果
- 如果不提供 `seed`，系统将使用随机种子，每次生成结果都不同

### 组图生成（豆包 Seedream 4.x 特有）

豆包 Seedream 4.0 和 4.5 支持组图功能，可以一次性生成多张风格一致的图片：
...
**参数说明**：
- `sequential_image_generation`: 设置为 `"auto"` 启用组图功能
- `sequential_image_generation_options.max_images`: 组图数量，范围 `1-4`，默认 `4`
- 组图功能会生成风格一致的多张图片，适合需要系列图片的场景

### 提示词优化（豆包 Seedream 4.0/4.5 特有）

豆包 Seedream 4.0 和 4.5 支持提示词优化选项，可以自动优化提示词效果：
...
**参数说明**：
- `mode: "standard"`: 标准模式，质量更高但耗时较长（默认，4.0 和 4.5 都支持）
- `mode: "fast"`: 快速模式，耗时更短但质量一般（仅 4.0 支持）
- `mode: "creative"`: 创意模式，更多创意发挥（仅 4.5 支持）
- `mode: "precise"`: 精确模式，更精确匹配提示词（仅 4.5 支持）

### 多图输入（GPT Image 特有）

GPT Image 支持同时输入多张图片（最多10张）进行融合生成：
...
**参数说明**：
- `images`: 多张输入图片数组，最多支持 10 张
- `input_fidelity`: 控制输入图片的保真度
  - `"low"`: 更多创意发挥，与原图差异较大
  - `"medium"`: 平衡保真度和创意
  - `"high"`: 保持原图特征，变化较小
  - `"auto"`: 自动选择合适的保真度
- `n`: 生成数量，范围 `1-10`，每张图片都会消耗相应的额度
...
**参数说明**：
- `parameters.n`: 生成数量，范围 `1-6`，默认 `1`。生成多张变体时按张计费
- `parameters.seed`: 随机种子，范围 `0-2147483647`，用于控制生成结果的随机性
- `parameters.negative_prompt`: 负面提示词，用于排除不想要的元素，如"模糊的文字、错别字"
- `parameters.watermark`: 是否添加水印，默认 `true`
