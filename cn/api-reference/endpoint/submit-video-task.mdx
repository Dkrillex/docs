...
<ParamField body="metadata.content" type="array" required>
  ...
  - `frames`：Seedance 1.5 pro 暂不支持duration 和 frames 二选一即可，frames 的优先级高于 duration。如果您希望生成小数秒的视频，建议指定 frames,取值范围：支持 [29, 289] 区间内所有满足 25 + 4n 格式的整数值，其中 n 为正整数。
  - `--fps` 或 `--framespersecond`：帧率，即一秒时间内视频画面数量。仅支持 `24`
  - `--seed`：随机种子取值范围：[-1, 2^32-1]之间的整数。
