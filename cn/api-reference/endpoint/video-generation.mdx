  ...
  <Tab title="豆包 Seedance">
    ...
    **5. 图生视频（参考图模式，仅 lite-i2v 支持）**
    ...
    curl -X POST "https://llm.ai-nebula.com/v1/video/generations" \
      ...
      -d '{
        "model": "doubao-seedance-1-0-lite-i2v-250428",
        "prompt": "[图1] 戴眼镜穿蓝色T恤的男孩和 [图2] 柯基狗，坐在 [图3] 草坪上，3D卡通风格",
        "content": [
          {
            "type": "text",
...
<ParamField body="content" type="array" required>
  内容数组，必须包含文本和可选的图片。参数通过文本提示词中的特殊标记控制：
...
<ParamField body="prompt" type="string" required>
  基础提示词（不含参数标记），参数标记会自动添加到 content 数组的文本中
</ParamField>
  </Tab>
</Tabs>
...
豆包 Seedance 系列模型使用特殊的参数传递方式：**所有参数通过提示词中的特殊标记传递**，图片通过 `content` 数组传递。
...
**重要提示**：
- 所有参数必须通过提示词中的特殊标记传递（如 `--ratio 16:9`）
